...
export default function Personal() {
  ...
  const handleDisconnectAccount = (accountId) => {
    Modal.confirm({
      ...
      content: "Are you sure you want to disconnect this account? You may lose access to some features.",
      okText: "Disconnect",
      okType: "danger",
      onOk: () => {
  ...
  return (
    ...
    >
          ...
          <Card className="content-section">
            ...
            <div style={{ display: "flex", alignItems: "flex-start", gap: 20, marginBottom: 24 }}>
              <div style={{ textAlign: "center" }}>
                ...
                >
                  <Text style={{ color: "white", fontWeight: "bold", fontSize: "24px" }}>
                    KV
                  </Text>
                </Avatar>
                {editMode && (
                    ...
                    <Col span={12}>
                      <Form.Item name="name" label="Full Name">
                        {editMode ? (
                          ...
                          <Text strong style={{ fontSize: 16 }}>{userProfile.name}</Text>
                        )}
                      </Form.Item>
                    </Col>
                    <Col span={12}>
      ...
      <Card className="content-section" style={{ marginTop: 24 }}>
        ...
        <Row gutter={[16, 16]}>
          {connectedAccounts.map((account) => (
            <Col xs={24} md={12} key={account.id}>
              <Card size="small" style={{ border: "1px solid var(--bs-border-color)" }}>
                ...
                <Space direction="vertical" style={{ width: "100%" }} size="small">
                  <div style={{ display: "flex", justifyContent: "space-between" }}>
                    <Text strong style={{ fontSize: "12px" }}>Connected:</Text>
                    <Text style={{ fontSize: "12px" }}>{account.connectedDate}</Text>
                  </div>
                  
                  {account.lastSync && (
                    <div style={{ display: "flex", justifyContent: "space-between" }}>
                      <Text strong style={{ fontSize: "12px" }}>Last Sync:</Text>
                      <Text style={{ fontSize: "12px" }}>{account.lastSync}</Text>
                    </div>
                  )}
                  
                  {account.events && (
                    <div style={{ display: "flex", justifyContent: "space-between" }}>
                      <Text strong style={{ fontSize: "12px" }}>Events:</Text>
                      <Text style={{ fontSize: "12px" }}>{account.events}</Text>
                    </div>
                  )}
                  
                  <div>
                    <Text strong style={{ fontSize: "12px" }}>Permissions:</Text>
                    <div style={{ marginTop: 4 }}>
                      {account.permissions.map((permission, index) => (
                        <Tag key={index} size="small" style={{ margin: "1px 2px" }}>
                          {permission}
                        </Tag>
                      ))}
                    </div>
        ...
        <Alert
          style={{ marginTop: 16 }}
          message="Account Integration"
          description="Your Google accounts are securely connected and enable automatic calendar synchronization, email notifications, and data sharing across integrated services."
          type="info"
